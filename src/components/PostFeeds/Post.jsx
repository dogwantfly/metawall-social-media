...
function Post({ post }) {
  ...
  const handleLike = async (postId) => {
    ...
    const apiMethod = isLiked ? unLikePost : likePost;
    try {
            ...
            >
              ...
              <div className="flex mb-5 gap-x-2">
                <button type="button" onClick={() => handleLike(post._id)} disabled={isLoading.like}>
                  {isLiked ? (<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#03438D" className="size-6 hover:fill-blue-custom/80">
              ...
              <div className="flex gap-x-2 mb-5">
                ...
                <button
                  type='button'
                  className='border-2 border-black-custom text-nowrap md:inline-block bg-blue-custom md:px-12 py-2 font-medium uppercase leading-normal text-white shadow-[0_4px_9px_-4px_rgba(51,45,45,0.7)] transition duration-150 ease-in-out  hover:shadow-[0_8px_9px_-4px_rgba(51,45,45,0.2),0_4px_18px_0_rgba(51,45,45,0.1)] hover:bg-yellow-custom focus:bg-yellow-custom focus:outline-none focus:ring-0 active:bg-yellow-custom hover:text-black-custom active:text-black-custom focus:text-black-custom'
                >
                  送出留言
                </button>
              </div>
