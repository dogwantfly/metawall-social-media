...
function Post({ post }) {
  ...
  const handleLike = useCallback(
    async (postId) => {
      const apiMethod = likes?.find((like) => like._id === user._id)
        ? unLikePost
        : likePost;
  ...
  const handleComment = useCallback(
    async (postId) => {
      try {
        ...
        setInputComment('');
        const sorted = (res.data.post.comments || []).sort(
  ...
  useEffect(() => {
    if (post) {
      ...
      setComments(sorted);
      setSortedComments(sorted.slice(0, 3));
    }
  }, [post]);
        ...
        >
          {post?.user && (
            <div className='flex items-center gap-4 mb-4 hover:text-blue-custom'>
              <Link
                to={{
                  pathname: `/personalPage/${post?.user?._id}`,
            ...
            >
              {likes?.find((like) => like._id === user._id) ? (
                <svg
                  ...
                  viewBox='0 0 24 24'
                  fill='#03438D'
                  className='size-6 hover:fill-blue-custom/80'
                >
                ...
                <svg
                  ...
                  strokeWidth={2}
                  stroke='#03438D'
                  className='size-6 hover:fill-blue-custom'
                >
                  <path
                    strokeLinecap='round'
          ...
          <div className='flex gap-x-2 mb-5'>
            ...
            >
              <img
                src={
                  user.avatar || 'https://tecdn.b-cdn.net/img/new/avatars/2.jpg'
                }
                alt={user.name}
                className='w-[45px] h-[45px] rounded-full border-2 border-black-custom'
              />
            ...
            <div className='relative mb-4 flex w-full items-stretch'>
              ...
              <button
                type='button'
                className={`border-2 border-black-custom text-nowrap md:inline-block md:px-12 py-2 px-2 flex-shrink-0 font-medium uppercase leading-normal  shadow-[0_4px_9px_-4px_rgba(51,45,45,0.7)] transition duration-150 ease-in-out hover:shadow-[0_8px_9px_-4px_rgba(51,45,45,0.2),0_4px_18px_0_rgba(51,45,45,0.1)] group  ${
                  isLoading.comment
                    ? 'bg-yellow-custom text-black-custom'
                    : 'bg-blue-custom text-white'
                } flex items-center ${
                  !inputComment.trim()
                    ? 'bg-blue-custom/80'
                    : 'hover:bg-yellow-custom focus:bg-yellow-custom focus:outline-none focus:ring-0 active:bg-yellow-custom hover:text-black-custom active:text-black-custom focus:text-black-custom'
                }`}
                onClick={() => handleComment(post._id)}
                disabled={isLoading.comment || !inputComment.trim()}
              >
                <p className='hidden md:block'>送出留言</p>
                ...
                <svg
                  ...
                  viewBox='0 0 24 24'
                  strokeWidth={2}
                  className={`md:hidden size-6 stroke-white ${
                    inputComment.trim() && 'group-hover:stroke-black-custom'
                  } `}
                >
                  <path
                    strokeLinecap='round'
          ...
          <ul className='mb-5'>
            {sortedComments?.length > 0 &&
              sortedComments.map((comment) => (
                <li
                  key={comment._id}
                  className='p-6 text-black-custom bg-gray-custom-light rounded-xl mb-4'
                >
                  <div className='flex items-center gap-4 mb-4'>
                    <Link
                      to={{
                        pathname: `/personalPage/${comment.user?._id}`,
                        state: { userId: comment.user?._id },
                      }}
                    >
                      <img
          ...
          {post.comments?.length > 3 && (
            <button
              type='button'
              className='text-blue-custom hover:text-yellow-custom'
